...
cp /root/mybot-master/decenterads/config/web/32cpu/default /etc/nginx/sites-available/default
...
#add cronjob
crontab -l > mycron
echo "*/30 * * * * php /root/mybot-master/decenterads/config/web/32cpu/kill.php > /dev/null 2>&1" >> mycron
...
#set chmod
chmod +x /root/mybot-master/decenterads/config/web/32cpu/*
#reboot
shutdown -r now