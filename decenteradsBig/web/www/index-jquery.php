...
function getProxy($ip){
    ...
    $db->open('inc/IP2PROXY-LITE-PX8.BIN', \IP2Proxy\Database::SHARED_MEMORY);
...
$randsize = array(480=>640,300=>250,320=>50);
$w = array_rand($randsize);
$h = $randsize[$w];
$data = array(
	"id" => $microtime.getRandNum(9),
	"imp" => array(
		array(
			"id" => $microtime.getRandNum(9),
			"banner" => array(
	...
	"app" => array(
		...
		"publisher" => array(
			"id" => md5(zone_id($bundle)),
			"name" => devName(zone_id($bundle)),
		),
	),
	"device" => array(
		"dnt" => rand(0,1),
	...
	"user" => array(
		"id" => md5(getRandAll(10)),
		"yob" => rand(1970,1999),
		"gender" => gender(),
	),
);
if(isset($get->location->latitude)){
    $data['device']['geo']['lat'] = $get->location->latitude;
}
if(isset($get->location->longitude)){
    $data['device']['geo']['lon'] = $get->location->longitude;
}
if(isset($get->city->name)){
    $data['device']['geo']['city'] = $get->city->name;
}
if(isset($get->mostSpecificSubdivision->isoCode)){
    $data['device']['geo']['region'] = $get->mostSpecificSubdivision->isoCode;
}
if(isset($get->postal->code)){
    $data['device']['geo']['zip'] = $get->postal->code;
}
...
<script type="text/javascript">
	console.log('<?=$console?>');
	var data_post = '<?=$json?>';
	$.post("http://bidder.decenterads.com/?token=9Xaw4VsmC3GPPfiyowK6", {
		...
		if(data == null){
			...
			var prebanner = unescape(data.seatbid[0].bid[0].adm);
			var banner = prebanner.replace('${AUCTION_PRICE}', data.seatbid[0].bid[0].price);
			$("#banner").append(banner);
			$.get("http://34.82.22.61/report.php?c=<?=$get->country->isoCode?>&ip=<?=$ip?>&data="+data.seatbid[0].bid[0].price+"&pkg=<?=$bundle?>&size=<?=$w.'-'.$h?>");
			console.log("Ad Show!");
		}
